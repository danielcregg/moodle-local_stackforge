...
/**
 * Thin adapter over Moodle's core AI subsystem (\core_ai) so the forge can reuse a site's configured
 * AI provider to draft an expression, instead of needing its own API key.
 *
 ...
 * site's configured provider, so the API key, provider management and central logging are Moodle's. The
 * AI only ever proposes a source expression here (the oracle still validates it), and the model answer
 * is never involved. Everything is feature-probed and wrapped: when core AI is unavailable this reports
 * so and the caller uses the plugin's own provider or a template default.
 *
 * Note: core's process_action() does not enforce the AI User Policy, so the caller checks
 * policy_accepted() before generating and records acceptance only on an explicit user action.
 */
class core_ai {
    /**
    ...
    public static function policy_text(): string {
        ...
        return $raw === '' ? '' : trim(html_to_text($raw, 0, false));
    }
    ...
    public static function generate_text(\context $context, int $userid, string $prompt): ?string {
        try {
            ...
            if (!$response->get_success()) {
                debugging('local_stackforge core AI failed: ' . $response->get_errormessage(), DEBUG_DEVELOPER);
                return null;
            }
            $text = trim((string) ($response->get_response_data()['generatedcontent'] ?? ''));
            return $text === '' ? null : $text;
        } catch (\Throwable $e) {
            debugging('local_stackforge core AI error: ' . $e->getMessage(), DEBUG_DEVELOPER);
            return null;
        }
    }
}
